...
Background run (survives terminal/IDE close):
    nohup /path/to/python -u matrixkernle_optimmiz.py > /tmp/matrixkernle_optimmiz.log 2>&1 &
    disown
  Use the conda env's Python directly (not `conda run`) to avoid stdout buffering.
  Check log:  tail -f /tmp/matrixkernle_optimmiz.log
  Stop:       kill $(pgrep -f matrixkernle_optimmiz)
"""
