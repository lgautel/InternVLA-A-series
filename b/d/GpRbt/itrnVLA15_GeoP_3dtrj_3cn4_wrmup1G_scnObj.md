# InternVLA-A1.5 + GeoPredict：Keypoint Expert 单卡 Warmup — `scan_object`

> **文档定位**: 在 [v3.4 设计手册](itrnVLA15_GeoP_3dtrj_3cn4.md) 与 [kptsim Warmup 方案](itrnVLA15_GeoP_3dtrj_3cn4_wrmup.md) 基础上，给出 **RoboTwin 2.0 `scan_object`** 任务上、**单卡（1G）** 对 Keypoint Expert 做 Phase 1 Warmup 的完整可执行方案。
>
> **数据前提**: kptsim 3D GT 已由 GeoPredict SAPIEN FK 提取完成（2026-08-25），见 [`GeoPredict/b/d/3dkptraj_1_scnObj_hngMg_LOG.md`](../../GeoPredict/b/d/3dkptraj_1_scnObj_hngMg_LOG.md)。本文 **不重跑提取**。
>
...
- [附录 C：ep42 步长 caveat](#附录-cep42-步长-caveat)
...
| [GeoPredict `3dkptraj_1_scnObj_hngMg_LOG.md`](../../GeoPredict/b/d/3dkptraj_1_scnObj_hngMg_LOG.md) | `scan_object` kptsim 提取日志 | GT 溯源、offset、ep42 |

姐妹手册：[hanging_mug 1G Warmup](itrnVLA15_GeoP_3dtrj_3cn4_wrmup1G_hngMg.md)。**两任务 offset / norm stats / lrbv30 均独立，禁止混用。**
...
```mermaid
flowchart TD
    A["kptsim npy 已存在<br/>scan_object_kptsim/"] --> B["compute 任务专属 norm_stats"]
    B --> C["inject_kptsim_keypoints.py<br/>→ scan_object_kptsim_lrb"]
    C --> D["Layer 1 静态验收 6 项"]
    D --> E["v2.1 转 v3.0"]
    E --> F["rsync → scan_object_kptsim_lrbv30"]
...
| 项 | 值 |
|:---|:---|
| 任务名 | `scan_object`（扫描物体：抓取 + 移动） |
| `TASK_NAMES` 索引 | **41**（[`inference.py`](../evaluation/RoboTwin/inference.py)） |
...
| 资产 | 路径 | 说明 |
|:---|:---|:---|
| LeRobot 主数据 | `/home/luogang/share/zwy/Projects/DATA/RoboTwin-Clean/scan_object/` | v2.1，50 ep，**8463** frames，15 fps |
| kptsim 关键点 GT | `/home/luogang/share/zwy/Projects/DATA/RoboTwin-Clean/scan_object_kptsim/` | 50 ep，`keypoints.npy` `[T, 42]`，2026-08-25 提取 |
...
\[
\mathbf{o} = [-0.6748,\ -1.0345,\ 0.6219]
\]

变换后范围：min \([0.323,\ 0.376,\ 0.157]\)，max \([1.277,\ 1.224,\ 0.843]\)，均在 \([0, 1.6]\times[0, 1.6]\times[0, 1.0]\) 内。

### 1.4 本文将生成的产物（尚不存在）

| 产物 | 路径 |
|:---|:---|
| 任务专属 norm stats | `/home/luogang/SRC/Robot/GeoPredict/ckpts/robotwin_norm_stats_scan_object.json` |
| 注入 v2.1 | `/home/luogang/share/zwy/Projects/DATA/RoboTwin-Clean/scan_object_kptsim_lrb/` |
| **训练用 v3.0（推荐）** | `/home/luogang/share/zwy/Projects/DATA/RoboTwin-Clean/scan_object_kptsim_lrbv30/` |
...
**禁止**把 `stack_bowls_three` 的 offset、norm stats、`keypoints_meta.json`、lrbv30 路径套到本任务。

| 维度 | `scan_object`（本文） | `stack_bowls_three`（对照） |
|:---|:---|:---|
| 总帧数 | **8463** | 23550 |
| \(\mathbf{o}\) | \([-0.675,\ -1.035,\ 0.622]\) | \([-0.812,\ -1.024,\ 0.505]\) |
| 体素 min | \([0.323,\ 0.376,\ 0.157]\) | \([0.405,\ 0.365,\ 0.253]\) |
| 体素 max | \([1.277,\ 1.224,\ 0.843]\) | \([1.195,\ 1.235,\ 0.747]\) |
| `TASK_NAMES` | **41** | 46 |
| norm stats | **必须新算** `..._scan_object.json` | `robotwin_norm_stats.json`（仅该任务） |
| `validate_all` 5 cm | ep42 右 TCP \(0.125\,\mathrm{m}\)，**数据仍可用**（[附录 C](#附录-cep42-步长-caveat)） | PASS |
...
```bash
test -f /home/luogang/share/zwy/Projects/DATA/RoboTwin-Clean/scan_object/meta/info.json && echo "SRC OK"
test -f /home/luogang/share/zwy/Projects/DATA/RoboTwin-Clean/scan_object_kptsim/keypoints_meta.json && echo "KPTSIM OK"
...
export NORM_RAW=/home/luogang/SRC/Robot/GeoPredict/ckpts/robotwin_norm_stats_scan_object.json
...
**不要**用默认输出路径 `ckpts/robotwin_norm_stats.json`（那是 stack_bowls_three）。
...
- 50 episodes，8463 frames
- XYZ 约 min `[0.323, 0.376, 0.157]`，max `[1.277, 1.224, 0.843]`
- 产物含 `observation.keypoint_3d`、`meta/info.json`（`keypoint_coord_mode=voxel`）、`norm_stat.json`、`meta/keypoints_meta.json`

```mermaid
flowchart LR
    subgraph src ["源（只读）"]
        LR["scan_object<br/>Parquet + MP4"]
        KPT["scan_object_kptsim<br/>keypoints.npy"]
        NS["norm_stats_scan_object.json"]
    end
    subgraph inject ["注入"]
        COPY["复制数据集"]
...
assert info['total_episodes'] == 50 and info['total_frames'] == 8463
print('Check 1 PASS')
"
```
...
DEST = Path('/home/luogang/share/zwy/Projects/DATA/RoboTwin-Clean/scan_object_kptsim_lrb')
KPTSIM = Path('/home/luogang/share/zwy/Projects/DATA/RoboTwin-Clean/scan_object_kptsim')
for i in range(50):
...
本任务 Z 最低约 \(0.157\)（低于 stack_bowls_three 的 \(0.253\)），仍 \(\ge 0\)。**不要**用 `validate_all` 的 5 cm 相邻帧阈值作为注入失败条件（见 [附录 C](#附录-cep42-步长-caveat)）。
...
assert meta['dataset_dir'].rstrip('/').endswith('scan_object')
...
转换产物：`${HF_LEROBOT_HOME}/robotwin/scan_object_kptsim_v30/`

- parquet 合并为 `data/chunk-000/file-000.parquet`（8463 行，含 `observation.keypoint_3d`）
- `codebase_version: v3.0`

复制溯源（转换脚本不会带上）：

```bash
V30=${HF_LEROBOT_HOME}/robotwin/scan_object_kptsim_v30
LRB=/home/luogang/share/zwy/Projects/DATA/RoboTwin-Clean/scan_object_kptsim_lrb
...
```bash
SRC=/home/luogang/.cache/huggingface/lerobot/robotwin/scan_object_kptsim_v30
DEST=/home/luogang/share/zwy/Projects/DATA/RoboTwin-Clean/scan_object_kptsim_lrbv30
rsync -a "$SRC/" "$DEST/"
test -f ${DEST}/meta/info.json && test -f ${DEST}/norm_stat.json && test -f ${DEST}/meta/keypoints_meta.json
```

训练 **只依赖** `scan_object_kptsim_lrbv30/`（parquet + video + meta + `norm_stat.json`），不再读外部 npy。
...
- `repo_id` = 目录名 `scan_object_kptsim_lrbv30`
...
ds = LeRobotDataset('scan_object_kptsim_lrbv30')
...
assert ds.num_episodes == 50 and ds.num_frames == 8463
print('Layer 2 PASS')
"
```
...
参考 [wrmup1G_LOG](itrnVLA15_GeoP_3dtrj_3cn4_wrmup1G_LOG.md)（stack_bowls_three 数量级，本任务帧更少，曲线形状应类似）：
...
```bash
grep -c '\[video_decode_error\]' outputs/internvla_a1_5/smoke_scan_object_kptsim_100step.log
grep -c 'using_zeros' outputs/internvla_a1_5/smoke_scan_object_kptsim_100step.log
```
...
1. 将 `scan_object_kptsim_lrbv30` 拷到/链到 `${VENV}/var/datasets/`
2. 把 [`launch/internvla_a15_geop_phase1_kpt_warmup_kptsim_8g.sh`](../launch/internvla_a15_geop_phase1_kpt_warmup_kptsim_8g.sh) 的 `DATA_REPO_ID` 改为 `scan_object_kptsim_lrbv30`
...
Phase 2 继续使用 **同一** `scan_object_kptsim_lrbv30`，坐标方案保持 voxel。
...
--kpt-meta-path /home/luogang/share/zwy/Projects/DATA/RoboTwin-Clean/scan_object_kptsim_lrbv30/meta/keypoints_meta.json
```
...
| 推理效果差 | 用了 stack_bowls 默认 meta | `--kpt-meta-path` 指向本任务 |
...
| 主数据（只读） | `.../RoboTwin-Clean/scan_object/` |
| kptsim GT（只读） | `.../RoboTwin-Clean/scan_object_kptsim/` |
| 注入 v2.1 | `.../scan_object_kptsim_lrb/` |
| **训练 v3.0** | `.../scan_object_kptsim_lrbv30/` |
| `HF_LEROBOT_HOME` | `.../RoboTwin-Clean` |
| `repo_id` | `scan_object_kptsim_lrbv30` |
| 原始 norm（注入输入） | `GeoPredict/ckpts/robotwin_norm_stats_scan_object.json` |
| 训练 norm | `.../scan_object_kptsim_lrbv30/norm_stat.json` |
| 推理 meta | `.../scan_object_kptsim_lrbv30/meta/keypoints_meta.json` |
| InternVLA base | `itvlaGp/ckpts/InternVLA-A1.5-base` |
| GeoPredict ckpt | `itvlaGp/ckpts/GeoPredict_robocasa.pth` |
| Smoke 输出 | `outputs/internvla_a1_5/smoke_scan_object_kptsim_100step/` |
| `task_idx` | **41** |
...
DATA_ROOT="${HF_LEROBOT_HOME}/scan_object_kptsim_lrbv30"
...
CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes=1 \
  ...
  --output_dir=outputs/internvla_a1_5/smoke_scan_object_kptsim_100step \
...
来源：[GeoPredict `3dkptraj_1_scnObj_hngMg_LOG.md` §4.2](../../GeoPredict/b/d/3dkptraj_1_scnObj_hngMg_LOG.md)。

GeoPredict `validate_all.py` 要求相邻帧每个关键点欧氏位移 \(< 0.05\,\mathrm{m}\)（该阈值来自 `stack_bowls_three` 实测最大步长 \(\approx 0.041\,\mathrm{m}\)）。
...
| 同帧最大 \(\lvert\Delta\theta\rvert\) | \(0.296\,\mathrm{rad}\)（约 \(17^\circ\)），**state[8] = 右肩 `right_shoulder`** |
| 左臂该帧 | \(\Delta\theta \approx 0\) |

右肩一帧大幅转动，经运动学杠杆放大到右 TCP \(\sim 12.5\,\mathrm{cm}\)，与演示轨迹一致，**不是提取脚本写错关节索引**。全任务仅 ep42 超过 5 cm。

**本文验收**：Layer 1 只检查 feature / 行对齐 / 体素范围 / 溯源，**不以 5 cm 阈值判定注入失败**。
