...
# ── Experiment name ───────────────────────────────────────────────────────
EXPR_NAME="${EXPR_NAME:-itvlagpFrkPlug0907_dtaug2}"
...
if [[ "${WAN_SMOKE}" == "1" ]]; then
    ...
    JOB_SUFFIX="frk-plug-sft-dtaug2-wan-smoke"
elif [[ "${SMOKE}" == "1" ]]; then
    export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"
    PROC_PER_NODE="${PROC_PER_NODE:-1}"
    BATCH_SIZE="${BATCH_SIZE:-2}"
    ...
    WANDB_ENABLE="${WANDB_ENABLE:-false}"
    JOB_SUFFIX="frk-plug-sft-dtaug2-smoke"
else
    ...
    JOB_SUFFIX="frk-plug-sft-dtaug2"
fi
...
ARGS=(
    ...
    # ── Image augmentation: ALL 6 transforms, affine at weight=0.2 ──
