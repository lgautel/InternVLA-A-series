...
###############################################################################
# Phase 2 SFT + Data Augmentation — Franka plug_into_socket 7D keypoints
#
# Based on: launch/frk_plug_sft_launch.sh (original Phase 2 SFT)
# Changes:  - Continues from SFT ckpt@10420 (not warmup ckpt)
#           - Enables image augmentation (all except affine, via disabled_tfs)
#           - EXPR_NAME=itvlagpFrkPlug0907_dtaug
#           - STEPS=50000, SAVE_FREQ=10000
#           - HF_HUB_OFFLINE + TRANSFORMERS_OFFLINE for expired token
#
# Usage:
#   bash b/s/Frk/frk_plug_sft_dtaug_launch.sh           # production 8 GPU
#   SMOKE=1 bash b/s/Frk/frk_plug_sft_dtaug_launch.sh   # 1 GPU smoke test
#   WAN_SMOKE=1 bash b/s/Frk/frk_plug_sft_dtaug_launch.sh  # WAN smoke
###############################################################################
...
# ── Experiment name ───────────────────────────────────────────────────────
EXPR_NAME="${EXPR_NAME:-itvlagpFrkPlug0907_dtaug}"
...
if [[ "${WAN_SMOKE}" == "1" ]]; then
    ...
    JOB_SUFFIX="frk-plug-sft-dtaug-wan-smoke"
elif [[ "${SMOKE}" == "1" ]]; then
    export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"
    PROC_PER_NODE="${PROC_PER_NODE:-1}"
    BATCH_SIZE="${BATCH_SIZE:-2}"
    ...
    WANDB_ENABLE="${WANDB_ENABLE:-false}"
    JOB_SUFFIX="frk-plug-sft-dtaug-smoke"
else
...
if [[ "${WAN_SMOKE}" == "1" || "${SMOKE}" == "1" ]]; then
    OUTPUT_DIR="${OUTPUT_DIR:-/tmp/sft_dtaug_smoke_frk_plug_${JOB_STAMP}}"
...
echo "=== Phase 2 SFT + Data Augmentation: Franka plug_into_socket 7D ==="
...
echo "Image augmentation: brightness/contrast/saturation/hue/sharpness (affine disabled)"
...
ARGS=(
    ...
    # ── Image augmentation (all except affine) ──
...
###############################################################################
# Monitoring helpers (adapted from launch/frk_plug_sft_launch.sh)
###############################################################################
...
_monitor_log "=== Franka SFT+DataAug automated monitoring enabled ==="
