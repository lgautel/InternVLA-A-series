...
###############################################################################
# Experiment B Phase 2: Main training with GeoPredict on R1 Pro
#
# Based on: launch/internvla_a15_geop_phase2_finetune_stackb3_080719.sh
# Changes:  num_keypoint_joints 14->16, dataset -> R1 Pro, history_max_len 300
# Pretrained from Phase 1 checkpoint (set PRETRAINED_PATH).
#
# KNOWN CONFOUND vs the baseline: this run inherits Phase 1's 400 steps on R1 Pro data,
# while the baseline starts from the base checkpoint. See the baseline script's header.
#
...
PRETRAINED_PATH="${PRETRAINED_PATH:?Set PRETRAINED_PATH to Phase 1 checkpoint, e.g. outputs/.../checkpoints/000400/pretrained_model}"
...
JOB_NAME="${JOB_NAME:-$(date +'%Y_%m_%d_%H_%M_%S')-${POLICY}-r1pro-geop-phase2-abs-${STEPS}}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJ_ROOT}/outputs/${POLICY}/${JOB_NAME}}"

echo "=== Experiment B Phase 2: GeoPredict Main Training ==="
echo "PRETRAINED_PATH=${PRETRAINED_PATH}"
echo "DATA=${DATA_REPO_ID}"
echo "OUTPUT_DIR=${OUTPUT_DIR}"
echo "STEPS=${STEPS} BS=${BATCH_SIZE}"
echo "loss: action=10 kpt=0.1 kpt_future=0.1 | expert-only, video_loss=0"
