...
###############################################################################
# GeoP Phase 2 smoke test: 100 steps on stack_bowls_three_kpt
# Self-contained venv: /tmp/itnvla15rbt20/
# From Phase 1 step 300 checkpoint (action-only, no WAN for faster smoke)
###############################################################################
...
export MASTER_PORT=${MASTER_PORT:-36400}
...
JOB_NAME="${JOB_NAME:-$(date +'%Y_%m_%d_%H_%M_%S')-${POLICY}-geop-phase2-smoke100-stackb3-abs}"
