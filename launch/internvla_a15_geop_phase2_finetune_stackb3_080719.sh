...
###############################################################################
# Phase 2 fine-tune (080719): Action + Kpt only
# VLM frozen (train_expert_only), no VLM loss (enable_vqa_loss=false)
# WAN loaded + frozen, no video loss (video_loss_weight=0.0)
# learnable_tokens trainable (freeze_learnable_tokens=false)
# 10000 steps, 8x H200, venv: /tmp/itnvla15rbt20/
###############################################################################
...
export MASTER_PORT=${MASTER_PORT:-36503}
...
JOB_NAME="${JOB_NAME:-$(date +'%Y_%m_%d_%H_%M_%S')-${POLICY}-geop-phase2-action-kpt-only-stackb3-abs-10k}"
