...
###############################################################################
# WAN smoke test (080718): 1 GPU, 2 steps
# loss weights: action=10, video=0.1, kpt=0.1, kpt_fut=0.1
###############################################################################
...
JOB_NAME="${JOB_NAME:-$(date +'%Y_%m_%d_%H_%M_%S')-${POLICY}-geop-phase2-wan-smoke-080718}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJ_ROOT}/outputs/${POLICY}/${JOB_NAME}}"

echo "WAN smoke 080718: action=10 video=0.1 kpt=0.1"
