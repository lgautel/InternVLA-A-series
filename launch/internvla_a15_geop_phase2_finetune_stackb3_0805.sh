...
###############################################################################
# Phase 2 fine-tune: InternVLA-A1.5 GeoP 3D fusion on stack_bowls_three_kpt
# From Phase 1 step 300 checkpoint, with video loss (WAN), kpt_loss_weight=1.0
# 20000 steps, save every 2500, 8x H200
###############################################################################
...
VENV_ROOT="${VENV_ROOT:-/tmp/itrnvla15rbt2}"
PYTHON="${PYTHON:-${VENV_ROOT}/bin/python}"
export LD_LIBRARY_PATH="/usr/local/nvidia/lib64:/opt/conda/lib:${VENV_ROOT}/lib:${LD_LIBRARY_PATH:-}"

export MASTER_ADDR=${MASTER_ADDR:-"127.0.0.1"}
export MASTER_PORT=${MASTER_PORT:-36300}
...
JOB_NAME="${JOB_NAME:-$(date +'%Y_%m_%d_%H_%M_%S')-${POLICY}-geop-phase2-action-video-kptw1-stackb3-abs-20k}"
...
echo "BATCH_SIZE=${BATCH_SIZE} STEPS=${STEPS} SAVE_FREQ=${SAVE_FREQ}"
