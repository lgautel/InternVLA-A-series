...
###############################################################################
# Smoke test (080719): 1 GPU, 2 steps — Action + Kpt only
# Expect: loss_vqa=0, loss_video=0, loss_action>0
###############################################################################
...
export MASTER_PORT=${MASTER_PORT:-36504}
...
JOB_NAME="${JOB_NAME:-$(date +'%Y_%m_%d_%H_%M_%S')-${POLICY}-geop-phase2-smoke-080719}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJ_ROOT}/outputs/${POLICY}/${JOB_NAME}}"

echo "Smoke 080719: action+kpt only, no vlm/video loss"
