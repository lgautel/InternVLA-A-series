...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def _query_videos(self, query_timestamps: dict[str, list[float]], ep_idx: int) -> dict[str, torch.Tensor]:
        ...
        for vid_key, query_ts in query_timestamps.items():
            ...
            try:
                ...
                logging.error(
                    "[video_decode_error] repo_id=%s ep_idx=%s vid_key=%s video_path=%s "
                    "from_timestamp=%s query_ts=%s shifted_query_ts=%s error=%r",
                    self.repo_id,
                    ep_idx,
                    vid_key,
                    video_path,
                ...
                base_shape = (c, h, w)
                fallback_shape = (len(shifted_query_ts), *base_shape) if len(shifted_query_ts) > 1 else base_shape
                frames = torch.zeros(fallback_shape, dtype=torch.float32)
                logging.warning(
                    "[video_decode_fallback] repo_id=%s ep_idx=%s vid_key=%s video_path=%s "
                    "using_zeros shape=%s",
                    self.repo_id,
                    ep_idx,
                    vid_key,
    ...
    def __getitem__(self, idx) -> dict:
        ...
        # Add task as a string. Some datasets expose coarse_task_index for task lookup.
        if "galaxea" in self.repo_id and "coarse_task_index" in item:
            task_idx = item["coarse_task_index"].item()
        else:
