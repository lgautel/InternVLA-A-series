#!/usr/bin/env python3
"""Acceptance tests for the two/six-server / six-EGL-client topology.

The production launcher defaults to six server instances (three per GPU,
``SERVER_INSTANCES_PER_GPU=3``); setting it to ``1`` falls back to the
original two-server topology:

    server GPU 0, port P       <- client GPUs 2, 3, 4
    server GPU 1, port P + 1   <- client GPUs 5, 6, 7

The mock-server live check below always uses two groups (it validates EGL
renderer/GPU isolation, which does not depend on how many server processes
share each model GPU). The static checks are safe to run anywhere.
``--live`` starts two *mock*
policy servers and six real LIBERO/EGL client processes.  Each client creates
an actual OffScreenRenderEnv on its assigned physical EGL device, sends
requests to its group's server, and completes several episodes in one child.
This tests the failure boundary that matters: EGL rendering and websocket
inference are concurrent, but no client renderer shares a GPU with a model
server.

The mock-server live test is intentionally separate from the full-model smoke:
it isolates the renderer/topology.  The full model smoke is run with
``run_eval_libero_plus_2server_6client_venv.sh EVAL_MODE=smoke``.

Usage:
    python evaluation/LIBERO-plus2/test_egl_split_topology.py

    export LIBERO_HOME=/home/a26113/DATA/LIBERO-plus
    export LIBERO_CONFIG_PATH=/tmp/test_libero_config
    python evaluation/LIBERO-plus2/test_egl_split_topology.py --live \
...
LAUNCHER = Path(__file__).with_name("run_eval_libero_plus_2server_6client_venv.sh")
...
def check_static() -> bool:
    ...
    checks = {
        ...
        "smoke_uses_six_shards": 'SHARDS_PER_SUITE="${NUM_CLIENTS}"' in src,
        ...
        "optimized_his_kpts_warning": "does not accept `his_kpts`" in src,
    }
...
def _ensure_config(libero_home: Path, config_path: str | None) -> Path:
    if config_path:
        ...
        return path
    path = Path(tempfile.mkdtemp(prefix="egl_split_config_"))
...
def _client_child(
    ...
    # OpenGL is queried only after the child has initialized its own EGL context.
    ...
    for episode in range(episodes):
        ...
        for step in range(steps):
            # The mock server exercises the same websocket request path.  The
            # dummy action keeps the smoke deterministic and short.
            response = client.predict_action(
                {
                    "type": "infer",
                    "request_id": f"split-smoke-{client_gpu}-{episode}-{step}",
    ...
    sys.stdout.flush()
    sys.stderr.flush()
    # Avoid a late interpreter GL destructor masking a successful render run.
    os._exit(0)
...
def run_live(args: argparse.Namespace) -> bool:
    ...
    root = Path(tempfile.mkdtemp(prefix="egl_split_live_"))
    ...
    try:
        ...
        # Wait for both websocket endpoints without importing mujoco in parent.
        ...
        while any(proc.poll() is None for proc in client_procs):
            if time.monotonic() >= deadline:
                raise TimeoutError("live client smoke timed out")
            time.sleep(1)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Two-server/six-EGL-client topology acceptance")
    ...
    parser.add_argument("--base_port", type=int, default=5874)
    ...
    print("[static] two-server/six-client EGL topology")
